...
def Rid_noise_(points, muki, char_size_px, sep_size_px):
    ...
    for combo in combinations:
        ...
        eps = abs(distance - char_size_px)
        if eps < sep_size_px * 0.1:
            is_equally_spaced = np.array([np.linalg.norm(combo[0] - point) > char_size_px * 0.9 for point in clusters])
            if is_equally_spaced.all():
                clusters.add(tuple(combo[0]))
            is_equally_spaced = np.array([np.linalg.norm(combo[1] - point) > char_size_px * 0.9 for point in clusters])
            if is_equally_spaced.all():
                clusters.add(tuple(combo[1]))
    if muki == 'tate':
        return sorted(list(clusters), key=lambda point: point[1])
